...
from warships.data import update_player_efficiency_data
from warships.models import Player


DEFAULT_STATE_FILE = Path(settings.BASE_DIR) / 'logs' / \
    'backfill_player_efficiency_badges_state.json'
...
class Command(BaseCommand):
    help = 'Backfill player efficiency badges with a durable checkpoint that can resume after interruption.'
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument(
            ...
            type=int,
            default=0,
            help='Also refresh players whose efficiency badges are older than this many hours.',
        )
        parser.add_argument(
            '--include-hidden',
            action='store_true',
        ...
        parser.add_argument(
            '--include-zero-pvp',
            action='store_true',
            help='Include players with zero PvP battles; by default the backfill only targets PvP-eligible players.',
        )
        parser.add_argument(
            '--force',
            action='store_true',
            help='Process all players in scope, even if they already have efficiency badges.',
        )
        parser.add_argument(
            '--max-errors',
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        include_zero_pvp = bool(options['include_zero_pvp'])
        ...
        def needs_efficiency_backfill(player: Player) -> bool:
            if force:
                return True

            if player.efficiency_json is None or player.efficiency_updated_at is None:
                return True

            if stale_cutoff is not None and player.efficiency_updated_at < stale_cutoff:
                return True
        ...
        for player in Player.objects.filter(id__in=pending_failures).order_by('id'):
            if should_stop():
                break
            try:
                update_player_efficiency_data(player, force_refresh=True)
        ...
        if not should_stop():
            ...
            if not include_zero_pvp:
                queryset = queryset.filter(pvp_battles__gt=0)
            ...
            for player in queryset.iterator(chunk_size=batch_size):
                ...
                if not needs_efficiency_backfill(player):
                    continue
                ...
                try:
                    ...
                    self.stderr.write(
                        f'Failed efficiency backfill for {player.name} ({player.player_id}): {error}')
        ...
        self.stdout.write(self.style.SUCCESS(
            'Efficiency badge backfill run complete: '
