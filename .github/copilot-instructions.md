...
For local SDLC in this repository, the optional agentic memory lane uses SuperLocalMemory in Mode A (math-only, zero LLM, local SQLite). Enable it with `BATTLESTATS_SLM_ENABLED=1`. The first call indexes the `agents/` markdown corpus lazily; subsequent calls only ingest changed files. See `agents/runbooks/archive/runbook-memory-layering-2026-04-10.md` for details. This path is droplet-safe but is opt-in via the same env var on the droplet.
