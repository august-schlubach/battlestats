...
| `gap_1d` | 24h-gap decomposition (added 2026-07-08): active-1d players with **no BattleEvent** in the window, split into `pvp_mover` (snapshot PvP delta > 0; sub-count `pvp_mover_no_event_48h` = no event in 48h either — **capture LATENCY, not loss**: verified 2026-07-11 these players are already in the pipeline and backfill on the next observation; a low-hundreds/day tail is expected, only a sustained RISE is a throughput signal), `non_pvp_active` (account clock moved, PvP battles flat — co-op/Operations, structurally invisible to PvP-only extraction; expected to dominate, NA highest), and `no_snapshot_pair` (unclassifiable). `null` until two snapshot days exist. **Reading these across 2026-07-20 → 2026-08-06:** the snapshot delta gate (`SNAPSHOT_DELTA_GATE_ENABLED`, live 2026-07-20) stopped writing a `Snapshot` row for players whose PvP stats did not move — exactly the `non_pvp_active` population — so the classifier lost its evidence and that bucket collapsed (18,380 → 64 overnight on 07-21) while `no_snapshot_pair` absorbed the difference. The gap itself never changed size. **Fixed 2026-08-06**: the classifier now reads `Player.activity_updated_at`, which the snapshot path refreshes on both its written and gate-skipped branches, so a checked-today player with no row is correctly `non_pvp_active` again. Snapshots captured **between 2026-07-21 and 2026-08-06 understate `non_pvp_active` and overstate `no_snapshot_pair`** — do not trend that bucket across the fix. |
