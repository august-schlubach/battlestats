...
| `battle-history-ships-color` | `{metric:'wr'\|'dmg'\|'kills'}` | Click a color-metric pill (WR% \| dmg \| Kills) in the battles× treemap header — one shared control recolors all three battle-history maps; default `wr`. Since 4.9.1 the pick is remembered **per player** (`bs:bh-ships-color:<realm>:<player>:<mode>`), so the event counts deliberate switches only — a restored pick fires nothing | `BattleHistoryTreemaps.tsx` (chooseColorMetric) | ✅ 53 (16) measured 2026-08-02 over 14d — `dmg` 24 (14) · `wr` 15 (12) · `kills` 14 (13) |
| `efficiency-sort` | `{realm, column:'name'\|'tier'\|'nation'\|'type'\|'award'\|'battles'\|'wr', direction:'asc'\|'desc'}` | Click a column header in the Efficiency badge table; `direction` is the resolved direction after the click. `column:'nation'` added 4.5.0 with the Nation column | `EfficiencyBadgeTable.tsx` (onSort) | ✅ 9 (2) live-verified 2026-07-26; `nation` 🟡 added 4.5.0, pending captures |
| `efficiency-filter` | `{realm, control:'tier'\|'type'\|'nation'\|'award'\|'clear', value, source:'dropdown'\|'treemap'\|'button'}` | Set an Efficiency filter. `source` (added 4.5.2) says which control was actually touched: a filter-bar dropdown, a **mini-treemap tile click** (toggles; `value` is the tile's value or `all` on toggle-off), or the **Clear button** (`source:'button'`, which also carries `control:'clear'`, `value:'all'`). Without `source` a tile click and a dropdown change were indistinguishable, so treemap usage could not be measured. `control:'nation'` added 4.5.1 with the Nation treemap + filter | `EfficiencyBadgeTable.tsx` (applyFilter/clearFilters) | ✅ 33 (5) live-verified 2026-07-26 (tier/type/award seen); `control:'nation'` 🟡 added 4.5.1, `source` 🟡 added 4.5.2 — both pending captures |
...
| `player-tier-chart-drilldown` | `{realm, source:'cell'\|'class'}` | Click a bar in the Profile tab's "Random Battles by Tier" figure (`cell` = one tier×class) or a class total beneath it (`class` = whole class, all tiers). Switches to the Ships tab and pre-filters it. `source` separates the two entry points so the class-total row can be judged on its own, the way `efficiency-filter` does | `PlayerDetailInsightsTabs.tsx` (openShipsFiltered) | 🟡 added 2026-07-27, pending deploy+captures |
