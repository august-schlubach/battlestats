...
3. **A budget that fits.** `RECAPTURE_TASK_OPTS` = **20 min soft / 21 min hard** since 2026-08-30 (was 15/16; asia was running at a median 89% of the 900s limit and truncating 3 days in 12 — `runbook-recapture-soft-limit-budget-2026-08-13.md`, Performed — 2026-08-30). Invariant: soft < hard ≤ the 6h `PLAYER_REFRESH_LOCK_TIMEOUT`, which does not move with the budget. The hard limit now **exceeds** the 20-min per-realm stripe gap, so two realms' sweeps can overlap; the lock is per-realm and the worker is `-c 3`, so that is a deliberate trade.
