...
- Per-ship `compute_ship_pop_avg_damage` (the request-driven gap warm) reads the SAME rollup since 2026-09-08: it sums `ShipPopDailyAgg` over the frozen days `[cutoff, frozen_end)` and raw-scans PDSS only over `[frozen_end, today]`, where `frozen_end = today - (SHIP_POP_ROLLUP_REFRESH_DAYS - 1)`. Gated on `ship_pop_rollup_covers_window`; on a gap it falls back to `_ship_pop_avg_damage_raw`, the extracted full-window scan that remains the identity oracle in tests. It never calls the catch-up — a viewer-driven warm must not write the rollup.
  - Why: the per-ship path had kept the whole-window raw scan the bulk path abandoned in July — 5-20s per popular ship, so a 20-ship warm held one of three `background` slots for ~9 minutes, inside the 00:00-00:15 UTC window where the snapshot chain and bulk warms are already queued (4 soft-limit kills in the 5 days to 2026-09-07, all in that quarter-hour). It also tripped the zero-successes detector on any day with no other dispatch.
...
`server/warships/tests/test_ship_pop_daily_rollup.py`: realm/mode partition correctness against a hand-seeded PDSS fixture; idempotency; re-roll picks up new rows; catch-up fills gaps, skips frozen days, refreshes trailing days, covers the window edge; output identity vs the raw per-ship computation (incl. pinned values, floor sentinel, banker's rounding); per-realm-scoped retention prune; second-run cheapness (frozen rows untouched). `TestComputeOneShipFromRollup` pins the per-ship fast path: the `frozen_end` seam counts the refresh day exactly once (the catch-up leaves it in BOTH sources, and a double count has no symptom but a quietly high average); values match the raw scan; a deleted frozen day falls back to it; the compute writes no agg rows; and a rollup row diverted from its PDSS source proves the fast path is actually taken rather than merely correct-by-fallback. The pre-existing end-to-end warm tests in `test_incremental_battles.py` now exercise the rollup path unchanged.
