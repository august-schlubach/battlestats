...
const EFFICIENCY_TIER_META: Record<'E' | 'I' | 'II' | 'III', { label: string; accentClassName: string; badgeClassName: string; }> = {
    III: {
        label: 'Grade III',
        accentClassName: 'text-[#8c4f1f]',
        badgeClassName: 'border-[#b87333] bg-[#fff1e6] text-[#8c4f1f]',
    },
    II: {
        label: 'Grade II',
        accentClassName: 'text-[#475569]',
        badgeClassName: 'border-[#94a3b8] bg-[#f8fafc] text-[#475569]',
    },
    I: {
        label: 'Grade I',
        accentClassName: 'text-[#946200]',
        badgeClassName: 'border-[#d4a72c] bg-[#fff7db] text-[#946200]',
    },
    E: {
        label: 'Expert',
        accentClassName: 'text-[#991b1b]',
        badgeClassName: 'border-[#b91c1c] bg-[#fff1f2] text-[#991b1b]',
    },
};

const HeaderEfficiencyRankIcon: React.FC<{ tier: 'E' | 'I' | 'II' | 'III'; percentile: number | null | undefined; populationSize: number | null | undefined }> = ({ tier, percentile, populationSize }) => {
    ...
    const description = `Battlestats efficiency rank ${meta.label}: ${percentileText}. Based on stored WG badge profile for ${populationText}.`;

    return (
        <span
            title={description}
            aria-label={description}
            className={`inline-flex cursor-help items-center gap-1 rounded-full border border-current/15 bg-white/85 px-2 py-1 text-[0.68rem] font-semibold uppercase tracking-[0.18em] shadow-sm ${meta.accentClassName}`}
        >
            <span className="text-[0.58rem] tracking-[0.22em] text-inherit/70">BST</span>
            <span className={`inline-flex min-w-[2.05rem] items-center justify-center rounded-full border px-1.5 py-0.5 text-[0.72rem] font-bold leading-none ${meta.badgeClassName}`}>
                {tier}
            </span>
        </span>
    );
...
const PlayerDetail: React.FC<PlayerDetailProps> = ({
    ...
    const efficiencyRankTier = !player.is_hidden
        ? (player.efficiency_rank_tier ?? (player.has_efficiency_rank_icon ? 'III' : null))
        : null;
    const hasEfficiencyRankIcon = !player.is_hidden && efficiencyRankTier !== null;
                    ...
                    <div className="mb-3 border-b border-[#c6dbef] pb-3">
                        <div className="flex flex-wrap items-start justify-between gap-3">
                            <div className="flex min-w-0 flex-wrap items-center gap-2">
                                ...
                                {hasEfficiencyRankIcon && efficiencyRankTier ? <HeaderEfficiencyRankIcon tier={efficiencyRankTier} percentile={player.efficiency_rank_percentile} populationSize={player.efficiency_rank_population_size} /> : null}
                            </div>
